...
export async function getStaffBookingByIdApi(bookingId: string) {
  const { data } = await apiClient.get<ApiResponse<ApiBooking>>(
    `/admin/bookings/${bookingId}`,
  )
  return data.data
}

export async function getStaffBookingsByCustomerApi(
  customerId: string,
  garageId?: string,
  limit = 50,
) {
  return getStaffBookingsApi({
